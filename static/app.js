(() => {
  ...
  function openFull(song = current()) {
    ...
    if (frame) {
      frame.innerHTML = `<iframe src="${youtubeUrl(song.yt)}" title="${String(song.title).replace(/"/g, '&quot;')}" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>`;
    }
    show(full);
  ...
  function filterCards(mood) {
    document.querySelectorAll('.song-card').forEach(c => {
      c.style.display = (mood === 'All' || c.dataset.mood === mood) ? '' : 'none';
    });
  }
  ...
  async function loadLocalSongsIfNeeded() {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        songs = data.map(s => ({
          id: Number(s.id), title: s.title, artist: s.artist, movie: s.movie,
          year: s.year, mood: s.category, yt: s.youtube_id || '', thumb: s.youtube_id ? `https://i.ytimg.com/vi/${s.youtube_id}/hqdefault.jpg` : ''
        }));
        if (songs.length) {
          currentIndex = Math.max(0, songs.findIndex(s => s.yt));
          updateUI();
          renderQueue();
        }
      }
  ...
  function bind() {
    ...
    document.querySelectorAll('.chip').forEach(ch => ch.addEventListener('click', () => {
      ...
      filterCards(ch.dataset.mood);
    }));

    document.querySelectorAll('.mood-card').forEach(c => c.addEventListener('click', () => {
      const ch = [...document.querySelectorAll('.chip')].find(x => x.dataset.mood === c.dataset.moodJump);
      if (ch) { ch.click(); $('collection')?.scrollIntoView({ behavior: 'smooth' }); }
    }));
    ...
    $('searchInput')?.addEventListener('input', e => {
      ...
      if (q.length < 2) {
        ...
        setText('searchStatus', 'Song, artist ya movie search karo.');
        return;
      }
      setText('searchStatus', 'Gaana dhoond rahe hain…');
      timer = setTimeout(async () => {
        try {
          // Local collection works even if the YouTube API key is not configured.
          const localMatches = songs.filter(s => `${s.title} ${s.artist} ${s.movie} ${s.year}`.toLowerCase().includes(q.toLowerCase())).slice(0, 12);
          let items = localMatches.map(s => ({ ...s, yt: s.yt || '', thumb: s.thumb || (s.yt ? `https://i.ytimg.com/vi/${s.yt}/hqdefault.jpg` : '') }));
          ...
          box.querySelectorAll('[data-search-index]').forEach(el => el.onclick = () => {
            const s = items[Number(el.dataset.searchIndex)];
            songs.push(s);
          ...
          setText('searchStatus', err.message || 'Search failed');
        }
      }, 300);
    });
    ...
    window.addEventListener('keydown', e => {
      if (e.key === 'Escape') {
        closeFull();
        document.querySelectorAll('.overlay').forEach(hide);
        hide($('queueDrawer'));
      }
    });
  }
  ...
  async function start() {
    // Bind UI immediately. The old version returned before bind() when the song array was empty.
    bind();
    await loadLocalSongsIfNeeded();
    if (songs.length) {
      const first = songs.findIndex(s => s.yt);
      currentIndex = first >= 0 ? first : Math.min(currentIndex, songs.length - 1);
      renderQueue();
      updateUI();
    }
  }
